...
# Clustering model
    ...
    params['popularity_threshold'] = popularity_threshold
...
# KNN
    ...
    top_courses = st.sidebar.slider('Top courses',
                                    min_value=0, max_value=100,
                                    value=10, step=1)
    k = st.sidebar.slider('Number of neighbors',
                          min_value=1, max_value=100,
                          value=40, step=10)
    user_based = st.sidebar.checkbox("User based", value=True)
    params['top_courses'] = top_courses
    params['k'] = k
...
# Regression with Embedding Features
    ...
    embedding_size = st.sidebar.slider('Embedding size',
                                       min_value=1, max_value=50,
                                       value=16, step=1)
    alpha = st.sidebar.slider('Alpha', min_value=0.0, max_value=100.0, value=10.0)
...
# Classification with Embedding Features
    ...
    embedding_size = st.sidebar.slider('Embedding size',
                                       min_value=1, max_value=50,
                                       value=16, step=1)
    max_depth = st.sidebar.slider('Maximum depth', min_value=0, max_value=100, value=10, step=1)
